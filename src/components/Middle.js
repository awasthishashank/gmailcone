    ...
    return (
        <div style={{ marginLeft: "2.9vw", width: '75vw' }}>
            ...
            {props.search ? mailData.filter((data) => data.sender === props.search).map((data) => (
                <Paper key={data.id} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)} elevation={0} style={{ backgroundColor: "#F8FCFF", borderBottom: "1px solid #EFEFEF", borderTop: "1px solid #EFEFEF" }}>
                    <ListItem>
                        ...
                        <span style={{ fontSize: "1.3vw", marginLeft: "1.2vw", fontWeight: "500" }}>{data.sender}
                            <span style={{ marginLeft: "12vw", fontWeight: "200", marginLeft: "1vw", cursor: "pointer" }}>{data.email}</span>
                        </span>
